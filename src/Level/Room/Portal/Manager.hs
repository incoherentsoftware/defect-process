...
updateRoomPortalManager portalMgr =
    let
        processInfoMsg :: [InfoMsgPayload] -> Maybe PosX
        processInfoMsg []     = Nothing
        processInfoMsg (p:ps) = case p of
            InfoMsgPlayer playerInfo -> Just $ vecX (playerInfoPos playerInfo)
            _                        -> processInfoMsg ps
    in do
        infoPlayerX <- processInfoMsg <$> readMsgs
        return $ portalMgr {_infoPlayerX = infoPlayerX}
